...
* UNITY_MIX ON/OFF, if ON the ASR ENV OUT is summed to the other ASR ENV OUt with UITY_MIX ON, and value is presented to the GLOBAL UNITY MIX ENV OUT output
...
pressing START all the VALUES will be presented at the ports, with correct timing expressed by the column MSECS
...
* BLACK input, is the CV in for the CV VCA left SLIDER
...
# DUAL FIVE STEPS
	...
	0.6.34
it's a copy of the Buchla 208 simple 5 steps sequencer, 2 times
...
# DUAL (208) ENVELOPE
	...
	0.6.35
it's a copy of the Buchla 208 envelope, 2 times
...
* EndOFCycle Pulse OUT (+LED): a 1 msec PULSE is coming out every time the cycle is closed - can be used to retrig the envelope to have an LFO/VCO
...
* DURATION, only working if in transient mode, is the time the ENV stays to 10v when the ATTACK phase is finished (time goes from 2 msecs to 10 secs)
...
# DUAL DUAL LPG
	...
	0.6.38
this is the continue of my Buchla 208 wanna be serie

the code is all based on the http://research.spa.aalto.fi/publications/papers/dafx13-lpg/DAFx2013-LPG.pdf (thanks to Julian Parker and Stefano D’Angelo !)
...
# TUNATHOR
	...
	0.6.38
a tool to map voltages for shaky voltages situation
...
when pressing START CALIBRATION button is trying to map voltages to traversing from C1 to C7 octaves
* using contexual menu it's possible to set up mapping mode to OCTAVE or SEMITONE (depends on the requested precision)
...
* CALIBRATION SIGNAL IN: the input for the target VCO freqeuncy detection
...
# ELSKER (the MULTITRIGDELAYER)
	...
	0.6.39
it's a 12 trig/gate delay with a common global TRIG, or single TRIGs
...
* single delayer
  ...
  * DURATION draggable display: from 1 msec to 999 seconds: length of delayed gate (gate ON is 10 Volts)
  * GATE out is the GATE trig out the yellow LED will indicate current VOLTAGE status

0.6.40

Added one shot mode: if active, the GLOBAL TRIG in will be valid only one time, and after the use the RESET button must be used to RESET the ONE SHOT mode

Added clock timing: the time of the clock in input is computed and used as timebase for the GATE and DELAY operations.

If ELSKER is timed, the field represents multiple of the base time

there is: a CLOCK input, a display showing the frequency of the clock and a display showing the time differential between pulses of the clock

Added Randomization to durations and delays: random is function of the current value in the field
> for example if the field contains 0.123 seconds, the random can generate a new value that in the 200% range of the current value (in this case from 0.001 to 0.246. if the delay is 0.000 the range will be considered 0.010
...
* RESET TRIG/TAP, the TRIG/TAP is output also as PULSE OUT to sync more devices will reset the timer to 0
* ALARMs section:
...
# P2V
	...
	0.6.39
an attempt for a more dedicated Pitch to Voltage module (the same used in HotTuna, but tuned more for Real Time action)
...
# GRAN TUNISMO
	...
	0.6.39
having had requests for a big display tuner, it's like HOTTUNA, but with less features, more precise
...
NYECHOECOECO up until 0.6.41, when changes were split off into its own module.
...
add an all gray disk for people don’t want running disks (check in YOURRACKINSTALL/plugins/nysthi/res/shapes/ and substitute the JIRAJIRADISK.svg)

	Addendum from Antonio (Edited for clarity)
"Regarding BIAS, it is the amount of current needed to overcome the recorded material.

If the Erase head is OFF, the magnetic disk contains relic recordings that are going to "fight" with incoming material with LEVEL "INPUT". The higher the BIAS, the higher the INPUT level must be to overcome the existing recorded material, but of course you are going to saturate!

The recording is always atomic, something is recorded (via swell/feedback) or not, The BIAS doesn't mix signal incoming and signals on disk, or one or the other, and can be from 1 single sample to 10 seconds, who knows!
